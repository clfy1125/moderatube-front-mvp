...
defineOptions({ name: 'ConfirmAnalysisModal' });
...
const handlePostChannelSpamAnalysis = async () => {
  try {
    await postChannelSpamAnalysis({ channelId: channelIdStore.channelId, post_action: 'none' });
...
<template>
  <ConfirmChannelAnalysisCommonModal v-slot="{ proformaInvoice }" title="전체 스팸 검사" post-action="none" :show-modal="showModal" @close="$emit('close')">
    <!-- 로딩 오버레이 -->
