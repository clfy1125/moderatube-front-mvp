...
defineOptions({ name: 'ConfirmVideosAnalysisAndDelModal' });
...
const handlePostVideosSpamAnalysis = async () => {
  try {
    await postVideosSpamAnalysis({ video_ids: props.videoIds, post_action: 'delete' });
...
<template>
  <ConfirmVideosAnalysisCommonModal v-slot="{ proformaInvoice }" :title="title" :video-ids="videoIds" post-action="delete" :show-modal="showModal" @close="$emit('close')">
    <!-- 로딩 오버레이 -->
